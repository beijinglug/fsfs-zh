...
-   以下注记自本文初次发表之后经过数次更新。
    ...
    Vista 同样赋予了微软额外的权力；例如，微软可以强制安装更新，它可以命令所有运行 Vista 的计算机拒绝运行某一特定的设备驱动程序。Vista 所施加的诸多限制的主要目的是强制实施用户所不能克服的数字限制管理（DRM）。DRM 的威胁正是我们发起 Defective by Design 运动的原因 [DefectiveByDesign.org](https://DefectiveByDesign.org)。
